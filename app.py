...
def show_question(q):
    ...
    if st.button("제출하기", type="primary", disabled=not answer.strip()):
        with st.spinner("AI가 피드백을 작성 중입니다..."):
            ...
            prompt = f"""당신은 중학교 1학년 국어 교사입니다. 모든 출력은 반드시 순수한 한국어로만 작성하세요. 한자, 중국어 간체자, 중국어 번체자, 일본어 히라가나, 일본어 가타카나, 영어 단어를 단 한 글자도 사용하지 마세요. 한국어 단어로 대체할 수 없는 경우에도 반드시 한국어로 풀어서 표현하세요. 황순원의 단편소설 「소나기」에 대한 학생의 서술형 답안에 피드백을 작성해 주세요.
...
3. 피드백 출력 형식 — 반드시 아래 두 부분으로 구성하세요:

[설명식 피드백]
학생 답변에 대한 전반적인 피드백을 2~3문장으로 작성하세요.
잘된 점, 보완할 방향, 심화 질문을 포함하세요.
