...
2. 보완할 점은 아주 구체적인 행동 지침으로 (막연한 지시 절대 금지!): 
   - "이유를 찾아보세요", "더 자세히 적어보세요" 같이 막연하게 말하지 마세요. 학생이 무엇을 써야 할지 바로 알 수 있도록 **지문에 나오는 구체적인 단어나 인물의 대사, 행동을 직접 따옴표('')로 인용**해서 명확한 행동 힌트를 주세요. 
   - (나쁜 예: "소녀의 기쁨이 커진 이유를 찾아서 답안에 덧붙여 볼까요?")
   - (좋은 예: "지문에서 허수아비가 '좀 전보다 더 우쭐거린다'고 표현된 부분을 활용해서, 소년과 함께 뛰놀면서 소녀의 마음이 얼마나 더 신나고 들떴는지 지금 쓴 답안에 한 문장으로 더 적어볼까요?")
...
(학생이 종이 학습지에 바로 옮겨 적을 수 있도록, 긴 문장 대신 짧고 명확한 핵심어 위주로 아주 간단하게 작성하세요.)
- 내가 잘한 점: (예: 소녀가 기뻐하는 모습을 정확하게 찾아낸 점)
- 나의 다음 행동: (학생이 답안에 무엇을 고치거나 추가해야 하는지 구체적으로 지시. 예: 답안에 '우쭐거린다'는 단어를 넣어서 소녀의 신난 마음을 한 문장으로 추가해 보기)
- 힌트가 되는 지문: (학생이 다시 읽어봐야 할 지문의 특정 행동이나 대사. 예: "좀 전 허수아비보다 더 우쭐거린다" 부분 다시 읽어보기)"""
