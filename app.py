...
1. 학생이 정답에 가까운 올바른 답변을 했을 경우:
   ...
   - 생각의 크기를 키워줄 수 있는 재미있고 쉬운 질문을 던지세요. (예: "만약 내가 소년이었다면 어땠을까요?")

2. 학생이 정답에서 벗어나거나 핵심을 놓쳤을 경우:
   - 절대로 정답을 직접 알려주지 마세요. 스스로 생각할 기회를 주어야 합니다.
   - "이 부분은 정말 잘 생각했어요! 그럼 여기서 소녀가 왜 그런 행동을 했는지 우리 조금만 더 생각해 볼까요?"처럼 다정하게 격려하며 힌트가 되는 질문을 던지세요.

[피드백 출력 형식 — 반드시 아래 두 부분으로만 구성하세요]

[설명식 피드백]
(여기에 학생 답변에 대한 전반적인 피드백을 2~3문장으로 다정하게 작성하세요. 잘된 점, 보완할 방향, 다시 생각해 볼 질문을 섞어서 자연스럽게 써주세요.)

[학습지에 바로 적을 수 있는 핵심 정리 ✏️]
- 잘된 점: (아주 쉽고 짧은 한 줄로)
- 보완할 점: (아주 쉽고 짧은 한 줄로)
- 다시 생각해 볼 질문: (학생이 스스로 고민해 볼 수 있는 쉬운 질문 한 가지)"""

            payload = {
                "model": "llama-3.3-70b-versatile",
                "messages": [{"role": "user", "content": prompt}],
                "max_tokens": 1000,
                "temperature": 0.5 # 답변의 안정성을 높이기 위해 추가
            }
            ...
            for attempt in range(max_retries):
                ...
                if res.status_code == 200:
                    ...
                    if "choices" in data:
                        ...
                        break
                elif res.status_code in [503, 429]: # Groq의 일시적 과부하 대응
