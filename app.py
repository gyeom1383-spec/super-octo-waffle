...
def show_question(q):
    ...
    if st.button("제출하기", type="primary", disabled=not answer.strip()):
        with st.spinner("인공지능 선생님이 꼼꼼하게 읽어보고 있어요..."):
            ...
            # [핵심] 순수 한글 피드백을 위한 강화된 프롬프트
            prompt = f"""당신은 중학교 국어 선생님입니다. 학생이 소설 '소나기'에 대해 쓴 답안에 피드백을 주세요.

[절대 규칙]
1. 모든 답변은 100% 한글로만 작성하세요. 
2. 영어 철자(A-Z), 한자(漢字)는 단 한 글자도 쓰지 마세요. 'OK', 'AI' 같은 단어도 절대 안 됩니다.
3. 어려운 문학 용어 대신 '앞으로 일어날 일에 대한 힌트'처럼 쉬운 우리말로 풀어서 쓰세요.
...
[피드백 구성]
- [선생님의 다정한 피드백]: 학생이 쓴 특정 단어를 칭찬하고, 지문의 특정 문장을 따옴표('')로 인용해서 어떻게 고치면 좋을지 친절하게 알려주세요.
- [한 걸음 더 나아가기 위한 나의 미션]: 잘한 점, 다음 행동(구체적 지시), 다시 볼 지문 내용을 짧고 명확하게 적어주세요.
...
반드시 한글만 사용하세요."""
            ...
            payload = {
                ...
                "max_tokens": 1000,
                "temperature": 0.2 # 답변의 일관성을 높이기 위해 더 낮게 조정
            }
