...
<u>돌아오는 길에는 열이틀 달이 지우는 그늘만 골라 짚었다. 그늘의 고마움을 처음 느꼈다.</u>""",
    ...
    },
    {
        ...
        "excerpt": """소녀가 분홍 스웨터 앞자락을 내려다본다. 거기에 검붉은 진흙물 같은 게 들어 있었다.<br>
소녀가 가만히 보조개를 떠올리며,<br>
"이게 무슨 물 같니?"<br>
소년은 스웨터 앞자락만 바라다보고 있었다.<br>
"내 생각해 냈다. 그날 도랑 건널 때 네게 업힌 일 있지? 그때 네 등에서 옮은 물이다."<br>
소년은 얼굴이 확 달아오름을 느꼈다.<br><br>
"글쎄 말이지. 이번 앤 꽤 여러 날 앓는 걸 약두 변변히 못 써 봤다더군. 지금 같애서는 윤 초시네두 대가 끊긴 셈이지……. 그런데 참 이번 계집애는 어린것이 여간 잔망스럽지가 않어. 글쎄 죽기 전에 이런 말을 했다지 않어? 자기가 죽거든 <u>자기 입던 옷을 꼭 그대루 입혀서 묻어 달라</u>구……." """,
...
- 힌트가 되는 지문:"""
    ...
    def generate():
        try:
            with requests.post(url, headers=headers, json=payload, stream=True, timeout=55) as res:
                full_text = ""
                    ...
                    if line:
                        ...
                        if line_str.startswith("data: "):
                            ...
                            try:
                                ...
                                chunk = chunk_data["candidates"][0]["content"]["parts"][0]["text"]
                                full_text += chunk
                                yield f"data: {json.dumps({'chunk': chunk}, ensure_ascii=False)}\n\n"
                            except:
                                continue
                if full_text:
                    log_to_sheets(student_class, student_group, q["label"], answer, full_text)
